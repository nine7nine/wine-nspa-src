 ...
 *
 * Queue semantics:
 *   - Bounded depth (NSPA_LF_CLOSE_QUEUE_CAP).  Push fails on overflow;
...
#ifndef __NSPA_LF_CLOSE_QUEUE_H
#define __NSPA_LF_CLOSE_QUEUE_H
...
#include <winternl.h>
#define NSPA_LF_CLOSE_QUEUE_SHARE_ALL (FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE)
...
extern BOOL nspa_lf_close_queue_push( HANDLE server_handle, int unix_fd );
...
extern void nspa_lf_close_queue_flush( void );

/* Pending count — diagnostic only. */
extern unsigned int nspa_lf_close_queue_pending( void );

#endif /* __NSPA_LF_CLOSE_QUEUE_H */
