...
static gboolean muxer_sink_event_cb(GstPad *pad, GstObject *parent, GstEvent *event)
{
    ...
    {
        case GST_EVENT_EOS:
            pi_mutex_lock(&muxer->mutex);
            muxer->eos = true;
            pi_mutex_unlock(&muxer->mutex);
            pi_cond_signal(&muxer->cond, &muxer->mutex);
            break;
...
NTSTATUS wg_muxer_create(void *args)
{
    ...
    muxer->offset = GST_BUFFER_OFFSET_NONE;
    pi_mutex_init(&muxer->mutex, 0);
    pi_cond_init(&muxer->cond, 0);
...
out:
    ...
    pi_cond_destroy(&muxer->cond);
    pi_mutex_destroy(&muxer->mutex);
    free(muxer);
...
NTSTATUS wg_muxer_finalize(void *args)
{
    ...
    pi_mutex_lock(&muxer->mutex);
    while (!muxer->eos)
        pi_cond_wait(&muxer->cond, &muxer->mutex);
    pi_mutex_unlock(&muxer->mutex);
