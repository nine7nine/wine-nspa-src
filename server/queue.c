...
/* DEFAULT-ON: block nspa_shared allocation for the process's first thread
 ...
 * msg-bypass feature is enabled: session_object_t / shared_object_t
 * allocation for the first thread's bypass ring disrupts the first
 * thread's message pump in ways not isolated by any runtime gate.  All
 * five server-subsystem gates + client IGNORE_LOCATOR + 64 KB ID stride +
 * poison-fill skip were tested and library still broke; only blocking
 * the allocation itself for first-thread queues fixes it.
 *
 * Set NSPA_MSG_BYPASS_ALLOW_MAIN=1 to opt back into first-thread
 * allocation (e.g. for root-cause investigation on a non-Ableton
 * workload).  Most apps' first thread is the GUI message pump; blocking
 * it by default is safe and preserves bypass for helper-thread peers. */
static int nspa_exclude_main_thread(void)
{
    static int cached = -1;
    if (cached < 0)
    {
        const char *v = getenv( "NSPA_MSG_BYPASS_ALLOW_MAIN" );
        cached = !(v && *v && *v != '0');
    }
    return cached;
}
...
static int nspa_ensure_shared( struct msg_queue *queue )
{
    ...
    if (nspa_exclude_main_thread() && nspa_queue_owner_is_process_first_thread( queue ))
    {
        static int probe = -1;
        if (probe == -1) probe = (getenv("NSPA_POST_DEBUG") != NULL);
        if (probe)
            fprintf( stderr, "nspa_post_debug: EXCLUDE_MAIN blocked alloc, caller_tid=%04x queue=%p\n",
                     current ? current->id : 0, queue );
        return 0;
    }
