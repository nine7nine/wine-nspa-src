...
/* NSPA msg-ring v2 Phase C Stage 3b: batched walker over input->msg_list.
 *
 * Mirrors get_hardware_message above with the following adjustments:
 *
 *   - Collects up to `max_count` fully-resolved hardware messages into
 *     out_entries[] instead of returning the first match.
 *   - Stops collecting (but continues walking for clear_bits/state
 *     maintenance) once `got_one` is set to preserve cross-thread FIFO,
 *     identical to the single-msg path.
 *   - Bytes used in out_entries is capped at max_reply_bytes (the
 *     caller's wineserver reply buffer size) to avoid an assert in
 *     set_reply_data; if the next entry would overflow we stop early
 *     and the next batch picks up via the unique_id continuation cursor.
 *   - PM_REMOVE handling is identical per-msg.
 *   - clear_queue_bits on end-of-walk only fires when nothing was
 *     collected AND ptr fell off the list, matching the single-msg
 *     path's "ran-through-and-found-nothing" semantics.
 *
 * Returns the number of entries written, OR -1 on error (set_error
 * already invoked by caller-visible mechanism).  Sets *bytes_used to
 * the byte count written to out_entries.
 *
 * See wine/nspa/docs/msg-ring-v2-phase-c-audit.md for the full audit. */
int nspa_get_hardware_msg_batch_walk( struct thread *thread,
                                      ...
                                      unsigned int *bytes_used )
{
    ...
    *bytes_used = 0;
    ...
    /* Position cursor — same continuation logic as get_hardware_message. */
    ptr = list_head( &input->msg_list );
    if (hw_id)
    {
        while (ptr)
        {
            struct message *m = LIST_ENTRY( ptr, struct message, entry );
            if (m->unique_id == hw_id) break;
            ptr = list_next( &input->msg_list, ptr );
        }
        if (!ptr) ptr = list_head( &input->msg_list );
        else      ptr = list_next( &input->msg_list, ptr );
    }
    ...
    while (ptr)
    {
        struct message *msg = LIST_ENTRY( ptr, struct message, entry );
        int msg_bit = get_hardware_msg_bit( msg->msg );
        struct nspa_hw_msg_batch_entry *entry;
        struct thread *win_thread;
        unsigned int msg_code;
        user_handle_t win;
        ...
        /* Reply-buffer budget vs caller-specified reply_max_size.
         * If adding this entry would overflow the caller's reply
         * buffer, stop batching — next call resumes via unique_id
         * continuation. */
        if (bytes + sizeof(*entry) > max_reply_bytes) break;

        /* Stamp unique_id and collect — identical to single path. */
        if (!msg->unique_id) msg->unique_id = get_unique_id();

        entry = &out_entries[returned];
        memset( entry, 0, sizeof(*entry) );
        entry->win    = win;
        entry->msg    = msg_code;
        entry->wparam = msg->wparam;
        entry->lparam = msg->lparam;
        entry->type   = MSG_HARDWARE;
        entry->x      = msg->x;
        entry->y      = msg->y;
        entry->time   = msg->time;
        memcpy( &entry->data, msg->data, sizeof(entry->data) );
        entry->data.hw_id = msg->unique_id;
        bytes += sizeof(*entry);
        returned++;

        /* Per-msg PM_REMOVE — identical to single path.  ptr was
         * advanced before this point, so list_remove inside
         * release_hardware_message doesn't invalidate our cursor. */
        local_flags = flags;
        if (msg_bit == QS_HARDWARE) local_flags |= PM_REMOVE;
        else if (!(msg_bit & (QS_RAWINPUT | QS_POINTER))) local_flags &= ~PM_REMOVE;
        if (local_flags & PM_REMOVE) release_hardware_message( current->queue, entry->data.hw_id );

        if (returned >= max_count) break;
    }

    /* End-of-walk clear_bits gating, identical to single path:
     * the original returned 0 + cleared bits only when "ran through
     * whole list, found nothing".  Preserve that semantics — when
     * we DID return msgs, the original would have early-returned
     * before reaching the clear-bits path. */
    if (returned == 0 && !ptr && clear_bits)
